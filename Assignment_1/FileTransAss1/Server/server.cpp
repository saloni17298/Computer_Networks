...
int main (int argc, char **argv)
{
 ...
 for ( ; ; ) {
  ...
  while ( (n = recv(connfd, buf, MAXLINELENGTH,0)) > 0)  {
   ...
   if (strcmp("ls\n",buf)==0)  {
	FILE *in;
	char temp[MAXLINELENGTH],port[MAXLINELENGTH];
	int datasock;
	...
	send(connfd, port,MAXLINELENGTH,0);				//sending data connection port no. to client
	datasock=accept_conn(datasock);	 			//accepting connection from client
	if(!(in = popen("ls", "r"))){
		cout<<"error"<<endl;
	}
	while(fgets(temp, sizeof(temp), in)!=NULL){
		send(datasock,"1",MAXLINELENGTH,0);
		send(datasock, temp, MAXLINELENGTH, 0);
		
	}
	send(datasock,"0",MAXLINELENGTH,0);
	pclose(in);
	//cout<<"file closed\n";
   }
   ...
   if (strcmp("pwd\n",buf)==0)  {
	...
	GetCurrentDir(curr_dir,MAXLINELENGTH-1);
	send(connfd, curr_dir, MAXLINELENGTH, 0);
	//cout<<curr_dir<<endl;
   }
   ...
   if (strcmp("cd",token)==0)  {
	...
	if(chdir(token)<0){
		send(connfd,"0",MAXLINELENGTH,0);
	}
	else{
		send(connfd,"1",MAXLINELENGTH,0);
	}
   }

   if (strcmp("put",token)==0)  {
	char port[MAXLINELENGTH],buffer[MAXLINELENGTH],char_num_blks[MAXLINELENGTH],char_num_last_blk[MAXLINELENGTH],check[MAXLINELENGTH];
	...
	sprintf(port,"%d",data_port);
	datasock=create_socket(data_port);				//creating socket for data connection
	send(connfd, port,MAXLINELENGTH,0);					//sending data connection port to client
	datasock=accept_conn(datasock);					//accepting connection
	recv(connfd,check,MAXLINELENGTH,0);
	cout<<check;
	if(strcmp("1",check)==0){
		...
		{
			recv(connfd, char_num_blks, MAXLINELENGTH,0);
			...
			recv(connfd, char_num_last_blk, MAXLINELENGTH,0);
   ...
   if (strcmp("get",token)==0)  {
	...
	sprintf(port,"%d",data_port);
	datasock=create_socket(data_port);				//creating socket for data connection
	send(connfd, port,MAXLINELENGTH,0);					//sending port no. to client
	...
	if ((fp=fopen(token,"r"))!=NULL)
	{
		...
		send(connfd, char_num_blks, MAXLINELENGTH, 0);
		...
		send(connfd, char_num_last_blk, MAXLINELENGTH, 0);
		if (num_last_blk > 0) { 
